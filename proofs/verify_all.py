...
- 1 FAIL
"""
import os, sys, json, subprocess, hashlib, datetime
...
def main():
    ...
    # V18.3.1
    v183=os.path.join(ROOT,"V18_3_1_ROOT_SEAL")
    ...
    # V18.7
    v187=os.path.join(ROOT,"V18_7_NONCIRCUMVENTION")
    res_path=os.path.join(v187,"results","results_v18_7.json")
    ok3,out3=run([sys.executable, os.path.join("checker","noncircumvention_checker.py"), "--N","200000","--out",res_path], cwd=v187)
    ...
    if ok3 and os.path.exists(res_path):
        ...
        # Expect no violations for E2
        v187_ok = (data.get("fuzz",{}).get("violations",{}).get("E2_no_allow_before_tau",1)==0)
    report["checks"]["V18_7_invariants"]={"pass":v187_ok}

    # V18.8
    v188=os.path.join(ROOT,"V18_8_CONVERGENCE_STABILITY")
    res_path2=os.path.join(v188,"results","results_v18_8.json")
    ok4,out4=run([sys.executable, os.path.join("checker","convergence_checker.py"), "--out",res_path2], cwd=v188)
    ...
    if ok4 and os.path.exists(res_path2):
        ...
        v188_ok = all([d.get("G1_determinism"), d.get("G2_no_allow_before_tau"), d.get("G3_convergence_at_tau"), d.get("G4_no_oscillation_without_boundary")])
    ...
    open(os.path.join(ROOT,"PROOFKIT_REPORT.json"),"w",encoding="utf-8").write(json.dumps(report,indent=2))
